...
void Ranger::display() const {
    ...
    for (int i = 0; i < arrows_.size(); i++) {
        std::cout << arrows_[i].type_ << ": ";
        std::cout << arrows_[i].quantity_ << std::endl;
    }
    std::cout << "Affinities: ";
    ...
    for (int i = 0; i < getAffinities().size(); i++) {
        if (getAffinities().size()-1 == i) {
            std::cout << getAffinities()[i];
        } else {
            std::cout << getAffinities()[i] << ", ";
        }
    }
    ...
    if (getRace() != "UNDEAD") {
        for (int i = 0; i < getAffinities().size(); i++) {
            if (getAffinities()[i] == "POISON") {
                setVitality(std::round(getVitality() / 2)); // If there is a POISON affinity, the vitality is divided by 2
                temp = true;
            }
        }

        if (getRace() != "UNDEAD" && temp != true) {
            setVitality(1);
        }

        if (getRace() != "UNDEAD" && getCompanion() == true) { // If the race is not UNDEAD and they have a companion, additional 1 vitality is added
