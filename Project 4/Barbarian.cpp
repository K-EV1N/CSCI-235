...
void Barbarian::eatTaintedStew () {
    if (getRace() == "UNDEAD") {
        ...
        toggleEnrage();
        if (getEnrage() == true) {
            setSecondaryWeapon("TABLE"); // Changes the secondary weapon to TABLE if they are enraged
        } else {
            setMainWeapon("BUCKET"); // If they don't have enrage on, they're main weapon is set to BUCKET
        }
    }
}