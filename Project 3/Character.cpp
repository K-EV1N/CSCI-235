...
bool Character::operator==(const Character& character2) const { // Checks to see if the character you inputted, has the same name, race, level and if they're an enemey or not
    if (name_ == character2.name_ && race_ == character2.race_ && level_ == character2.level_ && enemy_ == character2.isEnemy()) {
        return true;
    }
    return false;
}
...
bool Character::operator!=(const Character& character2) const { // Checks to see if your character is equal to or not equal to the other character
    if (name_ != character2.name_ || race_ != character2.race_ || level_ != character2.level_ || enemy_ != character2.isEnemy()) {
        return true;
    }
    return false;
...
void Character::display() const {
    ...
    if (enemy_ == true) {
        is_enemy = "They are an enemy";
    } else {
        is_enemy = "They are not an enemy";
    }
